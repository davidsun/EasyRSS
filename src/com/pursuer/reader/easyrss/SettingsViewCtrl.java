...
public class SettingsViewCtrl extends AbsViewCtrl implements OnSettingUpdatedListener {
    private static final boolean FOR_AMAZON = false;
    ...
    private void showSettingRate() {
        final View settingAbout = view.findViewById(R.id.SettingRate);
        settingAbout.setOnTouchListener(new OnTouchListener() {
            @Override
            public boolean onTouch(final View view, final MotionEvent event) {
                switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (!success) {
                        final Intent intent = new Intent(Intent.ACTION_VIEW, Uri
                                .parse("http://www.amazon.com/gp/mas/dl/android?p=" + context.getPackageName()));
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        try {
                            context.startActivity(intent);
                            success = true;
                    ...
                    if (!success) {
                        Toast.makeText(context, R.string.MsgCannotLaunchMarket, Toast.LENGTH_LONG).show();
                    }
                    break;
                case MotionEvent.ACTION_CANCEL:
                    view.setPressed(false);
